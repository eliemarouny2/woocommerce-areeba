...
class WC_AreebaWoocommerceGateway_CreditCard extends WC_Payment_Gateway
{
    ...
    {
        $this->id = AREEBA_WOOCOMMERCE_GATEWAY_EXTENSION_UID_PREFIX . $this->id;
        $this->method_description = AREEBA_WOOCOMMERCE_GATEWAY_EXTENSION_NAME . ' ' . $this->method_title . ' payments.';
        ...
        add_action('wp_enqueue_scripts', function () {
            wp_register_script('payment_js', $this->get_option('apiHost') . 'js/integrated/payment.min.js', [], AREEBA_WOOCOMMERCE_GATEWAY_EXTENSION_VERSION, false);
            wp_register_script('areeba_woocommerce_gateway_js_' . $this->id, plugins_url('/areebawoocommercegateway/assets/js/areeba-woocommerce-gateway.js'), [], AREEBA_WOOCOMMERCE_GATEWAY_EXTENSION_VERSION, false);
        }, 999);
    ...
    public function process_payment($orderId)
    {
         ...
         * gateway client
         */
        WC_AreebaWoocommerceGateway_Provider::autoloadClient();
        AreebaWoocommerceGateway\Client\Client::setApiUrl($this->get_option('apiHost'));
        $client = new AreebaWoocommerceGateway\Client\Client(
         ...
         * gateway customer
         */
        $customer = new AreebaWoocommerceGateway\Client\Data\Customer();
        $customer
        ...
        switch ($transactionRequest) {
            case 'preauthorize':
                $transaction = new \AreebaWoocommerceGateway\Client\Transaction\Preauthorize();
                break;
            case 'debit':
            default:
                $transaction = new \AreebaWoocommerceGateway\Client\Transaction\Debit();
                break;
        }
        ...
        if ($result->isSuccess()) {
            ...
            if ($result->getReturnType() == AreebaWoocommerceGateway\Client\Transaction\Result::RETURN_TYPE_ERROR) {
                // $errors = $result->getErrors();
                return $this->paymentFailedResponse();
            } elseif ($result->getReturnType() == AreebaWoocommerceGateway\Client\Transaction\Result::RETURN_TYPE_REDIRECT) {
                /**
            ...
            } elseif ($result->getReturnType() == AreebaWoocommerceGateway\Client\Transaction\Result::RETURN_TYPE_PENDING) {
                /**
                 * payment is pending, wait for callback to complete
                 */
            } elseif ($result->getReturnType() == AreebaWoocommerceGateway\Client\Transaction\Result::RETURN_TYPE_FINISHED) {
                /**
    ...
    public function process_callback()
    {
        ...
        if ($callbackResult->getResult() == \AreebaWoocommerceGateway\Client\Callback\Result::RESULT_OK) {
            switch ($callbackResult->getTransactionType()) {
                case \AreebaWoocommerceGateway\Client\Callback\Result::TYPE_DEBIT:
                case \AreebaWoocommerceGateway\Client\Callback\Result::TYPE_CAPTURE:
                    $this->order->payment_complete();
                    break;
                case \AreebaWoocommerceGateway\Client\Callback\Result::TYPE_VOID:
                    $this->order->update_status('cancelled', __('Void', 'woocommerce'));
                    break;
                case \AreebaWoocommerceGateway\Client\Callback\Result::TYPE_PREAUTHORIZE:
                    $this->order->update_status('on-hold', __('Awaiting capture/void', 'woocommerce'));
                    break;
            }
        } elseif ($callbackResult->getResult() == \AreebaWoocommerceGateway\Client\Callback\Result::RESULT_ERROR) {
            switch ($callbackResult->getTransactionType()) {
                case \AreebaWoocommerceGateway\Client\Callback\Result::TYPE_DEBIT:
                case \AreebaWoocommerceGateway\Client\Callback\Result::TYPE_CAPTURE:
                case \AreebaWoocommerceGateway\Client\Callback\Result::TYPE_VOID:
    ...
    public function init_form_fields()
    {
        $this->form_fields = [
            ...
            'apiHost' => [
                ...
                'default' => AREEBA_WOOCOMMERCE_GATEWAY_EXTENSION_URL,
            ],
            'apiUser' => [
                'title' => 'API User',
                'type' => 'text',
    ...
    public function payment_fields()
    {
        ...
        wp_enqueue_script('areeba_woocommerce_gateway_js_' . $this->id);
        ...
        <div id="areeba_woocommerce_gateway_errors"></div>
        <div class="payment_box" style="padding: 25px; background-color: #fff; border-radius: 3px; width: 450px; min-height: 330px">
            <div id="areeba_woocommerce_gateway_seamless">
                <input type="hidden" id="areeba_woocommerce_gateway_token" name="token">
                <p class="form-row form-row-wide" style="height: 80px;">
                    <label for="areeba_woocommerce_gateway_seamless_card_holder">Cardholder Name&nbsp;<abbr class="required" title="required">*</abbr></label>
                    <span class="woocommerce-input-wrapper">
                        <input type="text" class="input-text" id="areeba_woocommerce_gateway_seamless_card_holder" style="border-radius: 3px">
                    </span>
                </p>
                <p class="form-row form-row-wide" style="height: 80px;">
                    <label for="areeba_woocommerce_gateway_seamless_card_number">Card Number&nbsp;<abbr class="required" title="required">*</abbr></label>
                    <span class="woocommerce-input-wrapper">
                        <span id="areeba_woocommerce_gateway_seamless_card_number" class="input-text" style="padding: 0; width: 100%; border-radius: 3px"></span>
                    </span>
                </p>
                <p class="form-row form-row-first" style="height: 80px;">
                    <label for="areeba_woocommerce_gateway_seamless_expiry">Expiration Date&nbsp;<abbr class="required" title="required">*</abbr></label>
                    <span class="woocommerce-input-wrapper">
                        <input type="text" class="input-text" id="areeba_woocommerce_gateway_seamless_expiry" maxlength="5" placeholder="MM/YY" style="border-radius: 3px">
                    </span>
                </p>
                <p class="form-row form-row-last" style="height: 80px;">
                    <label for="areeba_woocommerce_gateway_seamless_cvv">CVC/CVV Code&nbsp;<abbr class="required" title="required" style="color: #b22222;
                    text-decoration: none;">*</abbr></label>
                    <span class="woocommerce-input-wrapper">
                        <span id="areeba_woocommerce_gateway_seamless_cvv" style="padding: 0; height: 52px; width: 187px; border-radius: 3px"></span>
                    </span>
                </p>   
            </div>
        </div>';
